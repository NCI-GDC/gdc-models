...
"""These are properties from the graph which should always be excluded from the overlay
mapping."""
...
class Structure:
    """An abstraction of a tree node representing a object in a mappings."""
    ...
    def _array_fields(self, path: Sequence[str] = ()) -> Sequence[str]:
        ...
        Returns:
            A sequence of fields which store array data. E.g.
    ...
    def to_mapping(self, include_meta: bool = True) -> dict[str, Any]:
        """Converts the tree structure to the mappings data.
...
class NodesStructureABC(Structure, abc.ABC):
    """An abstract structure which represents a node with properties from a dictionary node."""
    ...
    def to_mapping(self, include_meta: bool = True) -> dict[str, Any]:
        ...
        mapping["properties"] = dict(
            sorted({**self._extract_properties(), **mapping["properties"]}.items())
        )
...
_FILE_NODES = tuple(_get_nodes_by_category("index_file", "data_file"))
...
_IO_FILE = NodesStructure(nodes=_FILE_NODES, is_nested=True)
...
_FILE = NodesStructure(
    ...
    children=dict(
        ...
        index_files=NodesStructure(nodes=(models.File, *_FILE_NODES), is_nested=True),
